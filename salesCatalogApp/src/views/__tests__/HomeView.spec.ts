...
describe("HomeView.vue (HU4 y HU5)", () => {
  ...
  it("Renderiza correctamente el catalogo de productos (HU5)", async () => {
    ...
    expect(wrapper.text()).toContain("Catalogo de Productos")
  ...
  it("Verifica que no se pueda anadir a orden sin seleccionar cliente (HU4)", async () => {
    const notify = vi.fn()
    ...
    const wrapper = mount(HomeView, {
      props: { userName: "Piero", userId: "u1" },
    ...
    expect(notify).toHaveBeenCalledWith(expect.stringContaining("Debes seleccionar"), "warning")
  })

  it("Anade un pedido correctamente al seleccionar cliente (HU4 - Ventas Digitales)", async () => {
    const notify = vi.fn()
    ...
    expect(notify).toHaveBeenCalledWith("Crema anadido al pedido", "success")
  })

  it("Muestra el estado vacio si no hay productos disponibles", async () => {
    ;(global.fetch as any).mockImplementation((url: string) => {
      if (url.includes("/clients")) {
